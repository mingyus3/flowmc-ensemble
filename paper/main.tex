...
draws from the targets recovers about 93\% of the gap, and adding the
...
93\% of the gap between the ensemble and the matched single chain is
...
29.7\% (mean 18.4\%), and every configuration is significant at $p<0.05$. Read
...
two banana cases (within repetition noise), with 15\% to 44\% improvements on
...
had a partly different origin; see Section~\ref{sec:audit}.)
...
The 2{,}000-particle single chain sits at about 0.102 on average; on the
...
reweighting improves the mean JS by 6.1\% (best on the correlated Gaussian,
...
identical members performs essentially the same as the diverse one (0.0824
versus 0.0833, a difference inside the repetition noise), so member diversity
...
chain (mean JS 0.060 against 0.063) and sits far below the full ensemble
(0.083); it matches the single chain on every target within repetition noise,
...
the full stack lands within 7\% of the gap of the measured flowMC ensemble
...
$0.0806\pm0.0010$, which is 93\% of the measured flowMC ensemble's deficit
over the floor ($0.0833$). The pooled sample-set ESS, meanwhile, is blind to
...
of code once the generative form exists). Following this audit we corrected both generators and reran the three
affected configurations end to end (all budgets, ablations, and controls);
Tables~\ref{tab:control}--\ref{tab:rawpool} report corrected-reference
numbers throughout, and both the corrected generators and the rerun
per-repetition data are included in the repository [13]. Under the corrected
references every headline result is unchanged --- the matched single chain
still wins all three configurations (by 15--32\%) --- while the ring's
remaining residual (single chain 0.058 against a 0.044 floor) shows that,
after the reference artefact is removed, the ring retains a modest genuinely
sampler-limited component alongside the banana.
...
runs in about 14 seconds, comparable to the 2{,}000-particle chain (about 17
seconds; the difference is within run-to-run variation) and roughly six times
faster than the ensemble (about 84 seconds; the three corrected-reference
configurations were rerun on a different accelerator, which leaves the ratios
unchanged). The single chain therefore gets
...
Applied to exact samples, the two effects reproduce about 93\% of the measured
...
evaluating joint metrics on further flowMC outputs and repeating the study on
additional real posteriors with funnel-like or multimodal geometry.
...
applied to exact draws, the aggregation operators reproduce about 93\% of the
...
accounted for, only the banana --- and, to a smaller degree, the ring ---
remains genuinely sampler-limited. The
