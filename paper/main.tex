...
idea for making such a sampler more reliable is to run several of them with
different architectures and pool their samples. This paper began as an attempt
to validate exactly that: a five-member diverse flowMC ensemble equipped with
...
tempering schedule reproduces the measured ensemble values on the correlated
and funnel targets. The damage decomposes into a genuine distribution shift
...
not repair the aggregation. An audit of the reference generators additionally
shows that two targets were scored against a law that differs from their
potential, reframing which targets are genuinely sampler-limited. The
recommendation is unchanged and now mechanistic: run one flowMC chain at the
full budget; if members are pooled, pool them uniformly; and avoid
reweight-and-resample aggregation.
\end{abstract}
...
($|\mathrm{ED}|\le 0.002$ on every Table~5 entry, $\le 0.009$ for every individual repetition): resampling duplicates points but does
...
members, pool them uniformly and expect no gain over a single chain; stay away
