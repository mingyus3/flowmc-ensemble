...
# =============================================================================
# Section 6: raw-pooling 对照
# =============================================================================
def run_rawpool(label="rawpool_diverse@5x2000"):
    """跑5个diverse成员各2000，把1万个原始样本【直接拼接】(不重采样、不加权)算JS。
       同时也算一下full-method(重采样+加权)JS做交叉验证。"""
    palette=[dict(hidden_units=(64,64),n_layers=3,n_bins=8,n_local_steps=10,n_global_steps=10),
             dict(hidden_units=(96,96),n_layers=3,n_bins=12,n_local_steps=10,n_global_steps=10),
             dict(hidden_units=(128,64),n_layers=4,n_bins=8,n_local_steps=15,n_global_steps=8),
             dict(hidden_units=(64,128),n_layers=4,n_bins=10,n_local_steps=8,n_global_steps=15),
    ...
    for cfg in CONFIGS:
        ...
        ref=np.array(generate_reference_samples(fn,d,20000,**params))
        ...
        for rep in range(N_REPEATS):
            ...
            members=[]
            for m in range(5):
                c=palette[m]; sk,sub=jax.random.split(sk)
                s_m,_=flowmc_inference(sub,2000,d,
                    n_local_steps=c["n_local_steps"],n_global_steps=c["n_global_steps"],
                    n_training_loops=2,n_production_loops=2,n_epochs=5,
                    hidden_units=c["hidden_units"],n_bins=c["n_bins"],n_layers=c["n_layers"],
                    potential_fn=fn,**params)
                members.append(np.asarray(s_m))
            raw_pool=np.vstack(members)  # 10000个原始样本,均匀,不重采样
            js_raw=compute_marginal_js_distance(raw_pool,ref)
            ess_raw=compute_ess_d_dim(raw_pool)
            rows.append(dict(potential_fn=fn,dimension=d,repeat=rep,method="rawpool_uniform",
                             ESS=ess_raw,JS_distance=js_raw,n_particles=10000))
    return pd.DataFrame(rows)

def main():
    OUT="control_outputs"; os.makedirs(OUT,exist_ok=True)
    print("="*70); print("raw-pooling 对照 — 5个diverse成员的1万样本直接拼接(不重采样/不加权)"); print("="*70)
    df=run_rawpool()
    df.to_csv(os.path.join(OUT,"results_rawpool.csv"), index=False)
    print("\n[OK] 已保存 control_outputs/results_rawpool.csv，发我即可。")
    # 简单打印均值
    print("\n各配置 raw-pool JS 均值:")
    for (fn,d),g in df.groupby(["potential_fn","dimension"]):
        print(f"  {fn}-{d}: {g['JS_distance'].mean():.4f}")
    print(f"\n总均值 raw-pool JS = {df['JS_distance'].mean():.4f}")
    print("对比: single@10000均值=0.0649, ensemble(full)均值=0.0849")
    print(" * 若 raw-pool ≈ 0.065(接近single@10k) -> 'pooling没问题,是重采样/机制毁了它'")
    print(" * 若 raw-pool ≈ 0.085(接近full ensemble) -> ensemble思路本身就不如单链")
